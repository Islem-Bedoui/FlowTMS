...
export default function SuiviTourneesComponent() {
  ...
  useEffect(() => {
    ...
    const onFocus = () => {
      if (!cancelled) loadProofs();
    };
    const onVisibility = () => {
      if (document.visibilityState === 'visible' && !cancelled) loadProofs();
    };
  ...
  const startReturnsFlow = (city: string, orderNo: string) => {
    const params = new URLSearchParams();
    if (orderDate) params.set('date', orderDate);
    if (viewMode) params.set('view', viewMode);
    if (city) params.set('city', city);
    ...
    to.set('shipmentNo', orderNo);
    to.set('next', backUrl);
    router.push(`/retours-vides?${to.toString()}`);
  };
      ...
      <div className="container mx-auto p-3 md:p-6 max-w-7xl">
        <div className="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={reloadAll} className="px-3 py-2 rounded-lg bg-gradient-to-r from-sky-600 to-indigo-600 text-white shadow hover:from-sky-500 hover:to-indigo-500">Recharger</button>
          </div>
        </div>
                    ...
                    return (
                      <div key={o.No} className="grid grid-cols-1 gap-2 px-3 py-2 text-sm md:grid-cols-[1fr,auto] md:items-center">
                        ...
                        <div className="flex flex-wrap items-center gap-2 md:justify-end">
                          ...
                          {value === 'livre' && requireReturns && podOk && !returnsOk && (
                            <button
                              onClick={() => startReturnsFlow(city, o.No)}
                              className="px-2 py-1 text-[11px] rounded border bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-500"
                              title="Saisir les retours directement"
                            >Retours</button>
                          )}
                        </div>
                      </div>
